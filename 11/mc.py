...
def build_trigram_dict(wordlist):
    ...
    for i in range(0,len(wordlist)-2):
        ...
        #b = wordlist[i+1]
        #c = wordlist[i+2]
        #gives us the three words in three variables
...
def build_ngrams(wordlist,prelength):
    d = {}
    for i in range(0,len(wordlist)-prelength+1):
...
def generate_text(d,length=50):
    ...
    for i in range(length):
        ...
        if new_word == "":
            break
        next = next [i:] +(new_word,)
        #comma shows that its a tuple
